...
#include "position_tex_coord_color_matrix_vert_dxil.hpp"
...
namespace
{
    ...
    public:
        ...
        {
            auto vertex_shader = gpu::shader::create(device_,
                                                     {g_position_tex_coord_color_matrix_vert_dxil_data,
                                                      sizeof(g_position_tex_coord_color_matrix_vert_dxil_data)},
                                                     "main",
                                                     gpu::shader_format::dxil,
                                                     gpu::shader_stage::vertex,
                                                     gpu::shader_options{}.num_uniform_buffers(1));
            ...
            auto w = static_cast<float>(image.size().w());
            auto h = static_cast<float>(image.size().h());

            for (auto i = 0; i < 1000; ++i)
            {
                auto x0 = static_cast<float>(rand() % 1280);
                auto y0 = static_cast<float>(rand() % 720);
                auto x1 = x0 + w;
                auto y1 = y0 + h;

                auto alpha = static_cast<float>(rand() % 256) / 255.0f;

                vertices_.emplace_back(x0, y0, 0.0f, 0.0f, 1.0f, 1.0f, 1.0f, alpha);
                vertices_.emplace_back(x1, y0, 1.0f, 0.0f, 1.0f, 1.0f, 1.0f, alpha);
                vertices_.emplace_back(x0, y1, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, alpha);
                vertices_.emplace_back(x1, y1, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, alpha);

                indices_.emplace_back(i * 4 + 0);
                indices_.emplace_back(i * 4 + 1);
                indices_.emplace_back(i * 4 + 2);
                indices_.emplace_back(i * 4 + 2);
                indices_.emplace_back(i * 4 + 1);
                indices_.emplace_back(i * 4 + 3);
            }
            ...
            device_->acquire_command_buffer([this](gpu::command_buffer &cmd_buf) {
                ...
                cmd_buf.enqueue_render_pass(
                    *window_,
                    {0.3f, 0.4f, 0.5f, 1.0f},
                    *gfx_pipeline_,
                    [this, &cmd_buf](gpu::render_pass &render_pass, const size &size) {
                        auto ortho_matrix = util::create_2d_orthographic_matrix({1280, 720});
                        cmd_buf.push_vertex_uniform_data(0, ortho_matrix);

                        render_pass.bind_vertex_buffer(0, *vertex_buffer_);
                        render_pass.bind_index_buffer(*index_buffer_, gpu::index_element_size::bits_32);
                        render_pass.bind_fragment_sampler(0, *texture_, *sampler_);
                        render_pass.draw_indexed_primitives(static_cast<uint32_t>(indices_.size()));
                    });
            });
        }
...
int main()
{
    ...
    auto win = window::create(ctx, "Ortho 2D View", {1280, 720});
